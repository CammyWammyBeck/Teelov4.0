...
def test_compute_probabilities_uses_parent_path_keys(db_session):
    """Ensure scenario occurrence mass is allocated by (player_id, parent_node_id), not collapsed by player.

    This constructs an intentionally minimal/contrived bracket fragment where player 1 can reach the same
    QF slot via two distinct R16 parent nodes, and the QF prediction differs by that parent path.
    """

    t = Tournament(
        tournament_code="TESTPATH",
        name="Test Path",
        tour="ATP",
        gender="men",
        level="ATP 250",
    ...
    for pid, name in [(1, "P1"), (2, "P2"), (3, "P3"), (4, "P4"), (5, "P5"), (6, "P6")]:
        _make_player(db_session, pid, name)
    db_session.flush()

    # Underlying R32 matches feeding into a scenario R16 slot.
    m_r32_1 = Match(
        source="atp",
        tournament_edition_id=edition.id,
        round="R32",
    ...
    # Deterministic R16 pos2 actual match feeding the right side of QF pos1.
    m_r16_2 = Match(
        source="atp",
        tournament_edition_id=edition.id,
        round="R16",
    ...
    db_session.add_all([m_r32_1, m_r32_2, m_r16_2])
    db_session.flush()
    ...
    # Actual R32 nodes (these are the parents for the scenario R16 nodes).
    n_r32_1 = TournamentForecastNode(
        forecast_run_id=run.id,
        round="R32",
        draw_position=1,
        player_a_id=1,
        player_b_id=2,
        source_match_id=m_r32_1.id,
        node_type="actual",
        generation_depth=0,
        feature_set_name="fs",
        prediction_model_version="v",
    ...
    n_r32_2 = TournamentForecastNode(
        forecast_run_id=run.id,
        round="R32",
        draw_position=2,
        player_a_id=3,
        player_b_id=4,
        source_match_id=m_r32_2.id,
        node_type="actual",
        generation_depth=0,
        feature_set_name="fs",
        prediction_model_version="v",
    ...
    db_session.add_all([n_r32_1, n_r32_2])
    db_session.flush()

    # Scenario R16 pos1: two distinct matchups, both with player 1 on the left, different right opponent.
    # Both share the same R32 parents (n_r32_1, n_r32_2) but are distinct nodes (different player_b_id).
    n_r16_1_a = TournamentForecastNode(
        forecast_run_id=run.id,
        round="R16",
        draw_position=1,
        player_a_id=1,
        ...
        feature_set_name="fs",
        prediction_model_version="v",
        prediction_a=1.0,  # force player 1 win when this matchup occurs
        left_parent_node_id=n_r32_1.id,
        right_parent_node_id=n_r32_2.id,
    )
    n_r16_1_b = TournamentForecastNode(
        forecast_run_id=run.id,
        round="R16",
        draw_position=1,
        player_a_id=1,
    ...
    # Actual R16 pos2 (deterministic winner 5)
    n_r16_2 = TournamentForecastNode(
        forecast_run_id=run.id,
        round="R16",
        draw_position=2,
        player_a_id=5,
        player_b_id=6,
        source_match_id=m_r16_2.id,
        node_type="actual",
        generation_depth=0,
        feature_set_name="fs",
        prediction_model_version="v",
    ...
    db_session.add_all([n_r16_1_a, n_r16_1_b, n_r16_2])
    db_session.flush()

    # QF pos1: same nominal matchup (1 vs 5), but different *left parent path* => different win prob.
    n_qf_1 = TournamentForecastNode(
        forecast_run_id=run.id,
        round="QF",
        draw_position=1,
        player_a_id=1,
        ...
        prediction_model_version="v",
        prediction_a=0.9,
        left_parent_node_id=n_r16_1_a.id,
        right_parent_node_id=n_r16_2.id,
    )
    n_qf_2 = TournamentForecastNode(
        forecast_run_id=run.id,
        round="QF",
        draw_position=1,
        player_a_id=1,
        ...
        prediction_model_version="v",
        prediction_a=0.1,
        left_parent_node_id=n_r16_1_b.id,
        right_parent_node_id=n_r16_2.id,
    )

    db_session.add_all([n_qf_1, n_qf_2])
    db_session.commit()

    payload = compute_probabilities(db_session, edition_id=edition.id)
    assert payload["has_forecast"] is True

    # Player 1 reaches SF iff they win QF.
    # Occurrence mass of each R16 scenario path is:
    #   P(R32_1 produces 1) * P(R32_2 produces (3 or 4)) = 0.5 * 0.5 = 0.25.
    # There are two such paths, so total reach_qf for player 1 is 0.5.
    # But QF win is path-weighted:
    #   0.25*0.9 + 0.25*0.1 = 0.25.
    p1 = next(p for p in payload["players"] if p["player_id"] == 1)
    assert p1.get("reach_sf", 0.0) == pytest.approx(0.25, abs=1e-9)
