...
async def home(request: Request, db: Session = Depends(get_db), _feature_check: Optional[Any] = Depends(require_feature("enable_feature_matches"))):
    ...
    return templates.TemplateResponse("home.html", {"request": request, "upcoming_matches": [serialize_match(m) for m in upcoming], "completed_matches": [serialize_match(m) for m in completed], "stats": {"matches_total": db.query(func.count(Match.id)).scalar() or 0, "players_total": db.query(func.count(Player.id)).scalar() or 0, "editions_total": db.query(func.count(TournamentEdition.id)).scalar() or 0}, "now": datetime.utcnow(), "current_path": request.url.path})
