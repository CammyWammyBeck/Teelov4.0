...
<tr class="hover:bg-gray-50/50 transition-colors duration-75 group border-l-4 border-transparent hover:border-teelo-lime">
  ...
  <td class="px-5 py-3 text-right">${m.player_a?.id ? `<a href="/players/${encodeURIComponent(String(m.player_a.id))}" class="text-sm text-teelo-dark hover:underline decoration-teelo-lime decoration-2">${escapeHtml(m.player_a?.name || 'Unknown')}</a>` : `<span class="text-sm text-teelo-dark">${escapeHtml(m.player_a?.name || 'Unknown')}</span>`}</td>
  ...
  <td class="px-5 py-3">${m.player_b?.id ? `<a href="/players/${encodeURIComponent(String(m.player_b.id))}" class="text-sm text-teelo-dark hover:underline decoration-teelo-lime decoration-2">${escapeHtml(m.player_b?.name || 'Unknown')}</a>` : `<span class="text-sm text-teelo-dark">${escapeHtml(m.player_b?.name || 'Unknown')}</span>`}</td>
...
<div class="px-4 py-3 border-b border-gray-100 last:border-b-0">
  ...
  <div class="flex items-center justify-between gap-2">
    <div class="min-w-0 flex-1">
      <div class="text-[13px] text-teelo-dark truncate">${m.player_a?.id ? `<a href="/players/${encodeURIComponent(String(m.player_a.id))}" class="hover:underline decoration-teelo-lime decoration-2">${escapeHtml(m.player_a?.name || 'Unknown')}</a>` : `<span>${escapeHtml(m.player_a?.name || 'Unknown')}</span>`}</div>
      <div class="text-[13px] text-teelo-dark truncate">${m.player_b?.id ? `<a href="/players/${encodeURIComponent(String(m.player_b.id))}" class="hover:underline decoration-teelo-lime decoration-2">${escapeHtml(m.player_b?.name || 'Unknown')}</a>` : `<span>${escapeHtml(m.player_b?.name || 'Unknown')}</span>`}</div>
    </div>
